...
namespace Admin
{
    public partial class FarmerList : Form
    {
        SqlConnection con;
        ...
        private void LoadData()
        {
            ...
            {
                ConnectDatabase();
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Farmer]", con);
        ...
        private void button1_Click(object sender, EventArgs e)//Delete
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    // Insert deleted farmer into Delf table
                    SqlCommand insertCmd = new SqlCommand(
                        "INSERT INTO Delf (Name, Birth, Address, Division, [Postal Code], [Phone Number], Email, Gender, Password) " +
                    ...
                    // Delete from Farmer table
                    SqlCommand deleteCmd = new SqlCommand("DELETE FROM Farmer WHERE Id = @Id", con);
                    deleteCmd.Parameters.AddWithValue("@Id", selectedID);
                    deleteCmd.ExecuteNonQuery();

                    MessageBox.Show("Farmer deleted.");
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)//Search
        {
            ...
            if (!string.IsNullOrEmpty(searchValue))
            {
                ...
                {
                    ConnectDatabase();
                    string query = "SELECT * FROM [Farmer] WHERE [Name] LIKE @Search OR [Phone Number] LIKE @Search";
        ...
        private void FarmerList_Load(object sender, EventArgs e)
        {
            LoadData();
