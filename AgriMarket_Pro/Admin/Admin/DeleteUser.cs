...
namespace Admin
{
    public partial class DeleteUser : Form
    {
        ...
        private void LoadDeletedUsers()
        {
            ...
            {
                ConnectDatabase();
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Delu]", con);
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)//Search
        {
            ...
            if (!string.IsNullOrEmpty(searchValue))
            {
                ...
                {
                    ConnectDatabase();
                    string query = "SELECT * FROM Delu WHERE Name LIKE @Search OR [Phone Number] LIKE @Search";
        ...
        private void button5_Click(object sender, EventArgs e)//Add
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    // Insert back into UserList table
                    SqlCommand insertCmd = new SqlCommand(
                        "INSERT INTO UserList (Name, Birth, Address, Division, [Postal Code], [Phone Number], Email, Gender, Password) " +
                    ...
                    // Delete from Delu table
                    SqlCommand deleteCmd = new SqlCommand("DELETE FROM Delu WHERE Id = @Id", con);
                    deleteCmd.Parameters.AddWithValue("@Id", selectedID);
                    deleteCmd.ExecuteNonQuery();

                    MessageBox.Show("User restored.");
                    LoadDeletedUsers(); // Refresh the DataGridView
                    con.Close();
                }
                catch (Exception ex)
                {
            ...
            {
                MessageBox.Show("Please select a deleted user to restore.");
            }
        }
